...
function effectStyles(effect: string): string {
  if (effect === "restores")
    return "bg-sage/15 border-sage/40 text-sage-deep";
  if (effect === "elevates")
    return "bg-accent/10 border-accent/35 text-accent-deep";
  return "bg-white/45 border-white/70 text-ink/70";
}
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="eyebrow mb-2 text-accent-deep">Calendar</p>
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3 px-1 text-xs text-ink/55">
        <div className="flex flex-wrap gap-3">
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-sage/70" /> restores
          </span>
          <span className="flex items-center gap-1.5">
          ...
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-accent/60" /> costs
          </span>
        </div>
        {live.length > 0 ? (
          ...
          <a
            href="/settings"
            className="rounded-full border border-white/60 bg-white/40 px-3 py-1 font-medium text-ink/55"
          >
            Connect calendar →
          </a>
        )}
      ...
      <div className="glass-strong overflow-x-auto rounded-[28px] p-3 sm:p-4">
        <div className="min-w-[640px]">
          <div className="mb-2 grid grid-cols-[40px_repeat(7,1fr)] gap-1">
            <div />
            {days.map((day) => {
                ...
                {layoutDay(byDay.get(day) ?? []).map(({ event: e, col, cols }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`absolute overflow-hidden rounded-xl border px-2 py-1 text-left text-[11px] font-medium leading-tight ${effectStyles(e.impact.effect)}`}
      ...
      {selected && (
        ...
        >
          <div
            className="glass-strong w-full max-w-md rounded-[28px] bg-card/90 p-6"
